# forked from https://hg.nginx.org/nginx-tests/file/tip/lib/Test/Nginx.pm
...
sub DESTROY {
	...
	if (Test::More->builder->expected_tests) {
		my $errors = $self->read_file('error.log');
		$errors = join "\n", $errors =~ /.*unknown process.*/gm;
		Test::More::is($errors, '', 'no unknown process');
	}
...
sub test_globals_modules() {
	...
	$s .= "load_module $modules/ngx_http_cgi_module.so;\n";
